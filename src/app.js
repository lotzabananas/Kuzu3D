...
import { HandDebugHelper } from './components/HandDebugHelper.js';
...
class KuzuVRApp {
	constructor() {
		...
		logger.info('Initializing Kùzu VR App');
	}
	...
	async setupScene({ scene, camera, renderer, handTracking }) {
		...
		this.uiManager = new UIManager(scene, camera, renderer, handTracking);
		
		// Add debug helper to see if hands are tracked
		this.handDebugHelper = new HandDebugHelper(scene);
	...
	onFrame(delta, _time, { scene, handTracking }) {
		...
		// Update hand debug helper
		if (this.handDebugHelper && handTracking) {
			this.handDebugHelper.update(handTracking);
		}
		
		// Update hand tracking
		if (handTracking && this.nodeManager) {
			handTracking.update(
				scene,
				(object) => {
					const node = this.nodeManager.handleHover(object);
					this.uiManager.showNodeHover(node);
					return node;
				},
				(object) => {
					const node = this.nodeManager.handleSelect(object);
					if (node) {
						this.uiManager.showNodeDetail(node);
					}
					return node;
				}
			);
